...
class User extends Controller {
        ...
        if($check_user == null){
            ...
            */
            ...
            if($new_user->save()){
                ...
                return abort(200, 'save info error');
            }
        }else{
            return abort(200, 'username has existed');
        }
    }
    ...
    public function sign_in(){
        ...
        if($find_user == null){
            return abort(200, 'username does not exist' );
        }else if($find_user->password != $post_info['password']){
            //登陆时传来的密码也是加密过的密码
            return abort(200, 'password error');
        }else{
    ...
    public function update_playback(){
        ...
        if($result['code'] != 0){   //百家云返回的状态码为0表示成功
            abort(200, $result['msg'].'[code]:' . $result['code']);
        }
        ...
        foreach($list_playback as $playback){
            ...
            if(LivePast::get($playback['video_id']) == null && $playback['status'] == 100){
                ...
                if(!$live_now->save() || !$live_past->save()){
                    abort(200, $live_now->getError() . $live_past->getError());
                }
            }
        }
    }
    ...
    public function delete_operation($mode='history'){
        ...
        if($user == null){
            return abort(200, 'user does not exist');
        }
        if($mode == 'history'){ //删除history相当于退出课程/视频。
            switch ($post_info['deleteType']){
                case 'live':
                    ...
                    if($room ==null){
                        return abort(200, 'cannot find the live');
                    }else{
                ...
                case 'video':
                    ...
                    if($video ==null){
                        return abort(200, 'cannot find the video');
                    }else{
                    ...
                    break;
                default:
                    return abort(200, "deleteType cannot be ".$post_info['deleteType']);
            }
        }elseif ($mode == 'data'){
            if($user->role == 'student'){
                return abort(200, 'only teacher can delete data');
            }
            switch ($post_info['deleteType']){
                case 'live':
                    $room = LiveNow::get($post_info['videoId']);
    ...
    private function get_live_datafmt($lives, $base_url, $items_per_page, $current_page, $type=0){
        ...
        for($i = $start_index; $i < $start_index+$items_per_page && $i < $all_count; $i++){
            ...
            if($type == 0){
                ...
                if($playback == null){
                    return abort(200, 'playback not found');
                }
                $videos[] = [
                    'name' => $video->title,
                    //'hostName' => $teacher->real_name,
    ...
    private function get_videos($post_info, $base_url, $is_history){
        ...
        switch ($post_info['videoType']){
            ...
            case 'video':   //2017-12-28 16:22:34  这里的video原本多了个空格，导致匹配不上。
                ...
                break;
            default:
                return abort(200, 'videoType cannot be'.$post_info['videoType']);
        }
    }
            ...
            if($result_data['status']!=$video->status)
            {
                ...
                if($result_data['status']==100)
                {
                    ...
                    if($result_json2['code'] != 0){
                        return abort(200, $result_json2['msg'].'[code]:' . $result_json2['code']);
                    }
