...
class Room extends Controller {
    public function build_room(){
        ...
        if(true !== $image_check){
            return Funcs::myAbort(400, $image_check);
        }
        ...
        if($info){
            ...
            return Funcs::myAbort(502, $info->getError());
        }
        $parm = [
    ...
    public function enter_room($mode, $room_id, $role=0){
        ...
        if($role == 1){
            if(!Session::has('username')){
                return Funcs::myAbort(400, 'cannot enter as a teacher before you log in');
            }else if($user->role != 'teacher'){
                return Funcs::myAbort(400, 'cannot enter as a teacher when sign in as a student');
            }
        }
        ...
        if($mode == 'live'){
            ...
            if($role == 0 && Session::has('username')){
                ...
                try {
                    ...
                    return Funcs::myAbort(502,'add to history error'.$e->getMessage());
                }
            }
            $this->redirect($url);
            ...
            return Funcs::myAbort(400, 'mode is wrong');
        }
    }
}